...
Kromsys uses [Shiki](https://shiki.matsu.io) to do syntax highlighting at build
...
your Kromsys configuration (`next.config.js` file). Once it's enabled globally,
...
Kromsys uses CSS variables to define the colors for tokens. You can inject a
...
Kromsys configuration (`next.config.js` file).
...
`mdxOptions.rehypePrettyCodeOptions` option in your Kromsys config inside
`next.config.js`:
...
instead of [relying on CSS Variables](/docs/tbd):
...
Kromsys currently doesn't support specifying multiple themes. Because Shiki
